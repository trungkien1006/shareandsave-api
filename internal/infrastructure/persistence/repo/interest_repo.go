...
// 				// Lấy comment chưa đọc và sort theo thời gian tạo mới nhất
// 				return db.Order("created_at DESC").Offset(0).Limit(1)
// 			}).
// 			Preload("Author").
...
func (r *InterestRepoDB) GetAll(
	ctx context.Context,
	postInterests *[]interest.PostInterest,
	userID uint,
	filter interest.GetInterest,
) (int, int, error) {
	...
	if filter.Type == int(enums.InterestTypeInterested) {
		...
		query = query.Where("interest.user_id = ?", userID)
	} else {
		// Người khác quan tâm đến bài viết của user
		query = query.Where("post.author_id = ?", userID)
	}

	if filter.Search != "" {
		query = query.Where("(post.title LIKE ? OR user.full_name LIKE ?)", "%"+filter.Search+"%", "%"+filter.Search+"%")
	}
	...
	// Sắp xếp
	if filter.Sort != "" && filter.Order != "" {
		sortField := strcase.ToSnake(filter.Sort)
		if sortField == "created_at" {
			sortField = "post.updated_at"
		} else {
			sortField = "interest." + sortField
		}
		query = query.Order(sortField + " " + filter.Order)
	}
	...
	query = query.
		...
		Preload("PostItem.Item.Category").
		Preload("Interests", func(db *gorm.DB) *gorm.DB {
			sub := db.Where("deleted_at IS NULL")
			if filter.Type == int(enums.InterestTypeInterested) {
	...
	// Sắp xếp comment theo thời gian mới nhất
	for i := range dbPosts {
		sort.SliceStable(dbPosts[i].Interests, func(a, b int) bool {
			aComments := dbPosts[i].Interests[a].Comments
			bComments := dbPosts[i].Interests[b].Comments
	...
	// Mapping từ DB model sang domain
	unreadCount := 0
	for _, p := range dbPosts {
		for _, i := range p.Interests {
			for _, c := range i.Comments {
				if c.IsRead == 0 && c.SenderID != userID {
					unreadCount++
				}
			}
		}
		*postInterests = append(*postInterests, dbmodel.GetDTOToDomain(p))
	}

	totalPage := int((total + int64(filter.Limit) - 1) / int64(filter.Limit)) // làm tròn lên
