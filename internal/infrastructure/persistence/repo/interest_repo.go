...
func (r *InterestRepoDB) GetAll(ctx context.Context, postInterest *[]interest.PostInterest, userID uint, filter interest.GetInterest) (int, error) {
	...
	// Gán NewComment thủ công cho từng Interest
	for i := range dbPosts {
		for j := range dbPosts[i].Interests {
			interestID := dbPosts[i].Interests[j].ID

			var newComment dbmodel.Comment
			err := r.db.WithContext(ctx).
				Where("interest_id = ? AND is_read = 0 AND sender_id != ?", interestID, userID).
				Order("created_at DESC").
				Limit(1).
				First(&newComment).Error

			if err == nil {
				dbPosts[i].Interests[j].NewComment = append(dbPosts[i].Interests[j].NewComment, newComment)
			}
		}
	}

	// Sắp xếp lại Interests trong mỗi Post theo thời gian comment mới nhất
	for i := range dbPosts {
	...
	for _, post := range dbPosts {
		*postInterest = append(*postInterest, dbmodel.GetDTOToDomain(post))
	}
