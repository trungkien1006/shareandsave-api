...
func (r *PostRepoDB) GetAll(ctx context.Context, posts *[]post.PostWithCount, filter post.PostFilterRequest) (int, error) {
	...
	query = r.db.Debug().WithContext(ctx).
		...
		Where("post.status = ? OR post.status = ?", enums.PostStatusApproved, enums.PostStatusSeal).
		Group(`
...
func (r *PostRepoDB) GetAllUserPost(ctx context.Context, posts *[]post.PostWithCount, userID uint, filter post.PostFilterRequest) (int, error) {
	var (
		query   *gorm.DB
		results []dbmodel.PostWithCounts
	)
