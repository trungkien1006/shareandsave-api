...
func InitRoute(db *gorm.DB, redisClient *redis.Client) *gin.Engine {
	...
	notiUC := notificationapp.NewUseCase(notiRepo, notiService, redisRepo)
	...
	postUC := postapp.NewUseCase(postRepo, userRepo, rolePerRepo, postService, itemRepo, categoryRepo, notiService)
	...
	transactionUC := transactionapp.NewUseCase(transactionRepo, userRepo, interestRepo, itemRepo, postRepo, userGoodDeedRepo, rolePerRepo, notiService)
	...
	{
		//statistic API
		v1.GET("/statistics/transaction/:year", middlewares.AuthGuard, statisticHandler.StatisticTransactionInYear)
		...
		// user API
		v1.GET("/users/report", middlewares.AuthGuard, userHandler.GetUserReport)
		...
		v1.GET("/users/:userID/my-good-deeds", middlewares.AuthGuard, userHandler.GetUserGoodDeedByID)
