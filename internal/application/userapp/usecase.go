...
func (uc *UseCase) GetUserByID(ctx context.Context, users *user.User, userID int) error {
	if err := uc.repo.GetClientUserByID(ctx, users, userID, uc.clientID); err != nil {
		return err
	}
...
func (uc *UseCase) UpdateUser(ctx context.Context, domainUser *user.User) error {
	...
	if err := uc.repo.GetClientUserByID(ctx, &updateUser, int(domainUser.ID), uc.clientID); err != nil {
		return err
	}
...
func (uc *UseCase) DeleteUser(ctx context.Context, userID int) error {
	...
	if err := uc.repo.GetClientUserByID(ctx, &deleteUser, int(userID), uc.clientID); err != nil {
		return err
	}
