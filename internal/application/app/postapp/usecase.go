...
func (uc *UseCase) CreatePost(ctx context.Context, domainPost *post.CreatePost) error {
	...
	for index, image := range domainPost.Images {
		formatedImage, err := helpers.ProcessImageBase64(image, uint(enums.PostImageWidth), uint(enums.PostImageHeight), 100)
		if err != nil {
	...
	for key, oldItem := range domainPost.OldItems {
		...
		if oldItem.Image == "" {
			...
			strBase64Image, err := helpers.ProcessImageBase64(oldItem.Image, uint(enums.ItemImageWidth), uint(enums.ItemImageHeight), 100)
	...
	for key, newItem := range domainPost.NewItems {
		...
		if err != nil {
			if newItem.Image == "" {
				...
				strBase64Image, err := helpers.ProcessImageBase64(base64, uint(enums.ItemImageWidth), uint(enums.ItemImageHeight), 100)
				if err != nil {
					return err
				}
				...
				strBase64Image, err := helpers.ProcessImageBase64(newItem.Image, uint(enums.ItemImageWidth), uint(enums.ItemImageHeight), 100)
